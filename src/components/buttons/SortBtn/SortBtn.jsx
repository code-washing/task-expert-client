...
const SortBtn = ({ text = 'Sort', renderChildren, modifyClasses = '' }) => {
   ...
   return (
      <div className={`relative shadow-sm ${modifyClasses}`}>
         <button
            onClick={handleClick}
            className={`flex gap-2 items-center rounded-default bg-white hover:text-primary hover:border-primary border border-neutral-300 shadow-sm px-2 py-1 transition-all duration-default`}
         >
